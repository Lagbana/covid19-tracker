...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  createComparisonBarGraph("Canada", "USA", 36, 202);
...
function worldwideDataCollector() {
  ...
  worldwideData.casesPerOneMillion = Number(
    ((worldwideData.cases / 7713468000) * 1000000).toFixed(0)
  );
...
function displayTextData(countryName, countryIndex) {
  ...
  country.innerHTML =
    "Country: " + "<span class='base-text'>" + countryName + "</span>";

  let cases = document.getElementById("cases");
  cases.innerHTML =
    "Total Cases: " +
    "<span class='orange-text'>" +
    covidData.cases[countryIndex].toLocaleString() +
    "</span>";

  let casesToday = document.getElementById("today-cases");
  casesToday.innerHTML =
    "Cases Today: " +
    "<span class='orange-text'>" +
    " + " +
    covidData.todayCases[countryIndex].toLocaleString() +
    "</span>";

  let deaths = document.getElementById("deaths");
  deaths.innerHTML =
    "Total Deceased: " +
    "<span class='negative-data'>" +
    covidData.deaths[countryIndex].toLocaleString() +
    "</span>";

  let deathsToday = document.getElementById("today-deaths");
  deathsToday.innerHTML =
    "Deceased Today: " +
    "<span class='negative-data'>" +
    " + " +
    covidData.todayDeaths[countryIndex].toLocaleString() +
    "</span>";

  let recovered = document.getElementById("recovered");
  recovered.innerHTML =
    "Total Recovered: " +
    "<span class='positive-data'>" +
    covidData.recovered[countryIndex].toLocaleString() +
    "</span>";
  ...
  active.innerHTML =
    "Active Cases: " +
    "<span class='orange-text'>" +
    covidData.active[countryIndex].toLocaleString() +
    "</span>";
  ...
  critical.innerHTML =
    ...
    "<span class='orange-text'>" +
    covidData.critical[countryIndex].toLocaleString() +
    "</span>";

  let casesPerOneMillion = document.getElementById("cases-per-million");
  casesPerOneMillion.innerHTML =
    "Cases per Million: " +
    "<span class='orange-text'>" +
    covidData.casesPerOneMillion[countryIndex].toLocaleString() +
    "</span>";
}
...
function createComparisonBarGraph(
  countryNameOne,
  countryNameTwo,
  countryIndexOne,
  countryIndexTwo
  ...
  let casesPerOneMillionChartOne =
    covidData.casesPerOneMillion[countryIndexOne];

  let selectedCountryTwo = covidData.countries[countryIndexTwo];
  let casesPerOneMillionChartTwo =
    covidData.casesPerOneMillion[countryIndexTwo];
